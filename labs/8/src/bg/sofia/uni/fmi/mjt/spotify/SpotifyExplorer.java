...
public class SpotifyExplorer {
    ...
    public Map<Integer, Set<SpotifyTrack>> groupSpotifyTracksByYear() {
        var groupedTracks = tracks.stream()
    ...
    public int getArtistActiveYears(String artist) {
        Optional<Integer> min = tracks.stream()
                .filter((a) -> (a.artists().contains(artist)))
                .map(SpotifyTrack::year)
                .min(Comparator.naturalOrder());
        Optional<Integer> max = tracks.stream()
                .filter((a) -> (a.artists().contains(artist)))
                .map(SpotifyTrack::year)
                .max(Comparator.naturalOrder());
        if (min.isPresent() && max.isPresent()) {
            return max.get() - min.get() + 1;
        }
        return 0;
    }
    ...
    public List<SpotifyTrack> getTopNHighestValenceTracksFromThe80s(int n) {
        return Collections.unmodifiableList(tracks.stream()
    ...
    public SpotifyTrack getMostPopularTrackFromThe90s() {
        Optional<SpotifyTrack> track = tracks.stream()
                .filter((t) -> t.year() >= 1990 && t.year() <= 1999)
                .sorted(Comparator.comparingInt(SpotifyTrack::popularity).reversed()).findFirst();
        if (track.isPresent()) {
            return track.get();
        }
        throw new NoSuchElementException("No item matches.");
    }
    ...
    public long getNumberOfLongerTracksBeforeYear(int minutes, int year) {
        if (minutes < 0 || year < 0) {
            throw new IllegalArgumentException("Minutes or year are negative.");
        }
        return tracks.stream()
                .filter((t) -> t.duration() > minutes * 60 * 1000)
